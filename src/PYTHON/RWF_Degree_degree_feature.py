...
def reorder_adjacency_matrix(adj_matrix, k):
    # ranking by degree
    ...
    result = count_values_in_intervals(degrees, k)
    subset_indices = []
    for interval, indices in result.items():
        subset_indices.append(indices)
        # print(f"{interval}: {indices}")

    degrees = 1 + np.array(degrees)
    d_inv_sqrt = np.power(degrees, -0.5).flatten()
    d_inv_sqrt[np.isinf(d_inv_sqrt)] = 0.
    d_mat_inv_sqrt = sp.diags(d_inv_sqrt)
    return reordered_adj, subset_indices, d_mat_inv_sqrt.dot(reordered_adj).dot(d_mat_inv_sqrt).tocoo()
...
def extract_LM_embeddings_label(args, root, dataset):
    ...
    for i in tqdm(range(len(As))):
        print(i, end='\r')
        A = As[i]
        x_feat = np.maximum(1, np.asarray(np.sum(A, axis=1)))
        x_feat = normalize_to_0_1(x_feat)
        num_nodes = A.shape[0]
        num_edges = A.count_nonzero()
        A_new, X, P = reorder_adjacency_matrix_with_x(A, x_feat)
...
def main():
    ...
    print("exp with degree partitioning, with node degree as feature")
    ### COLLAB PROTEINS
    ...
    parser.add_argument('--embedder', type=str, default='LM',
    ...
    for args.tau in [3,4,5,6,7,8,9,10]:
        ...
        embedding_file = args.dataset + '_LM' + '_' + str(args.k) + '_' + str(args.tau) + '_D_degree_feature.csv'
        ...
        file_out = '../../results/' + args.dataset + '_' + str(round(best_mean[idx],3)) + '_' + str(args.k) + '_' + str(args.tau) + '_D_with_degree_feature.txt'
    ...
    print("exp with degree partitioning, with node degree as feature, varying tau: ")
    print(best)
    best_file = '../../results/' + args.dataset + '_' + str(args.k) + '_D_degree_feature_results.txt'
