...
def extract_LM_embeddings_label(args, root, dataset):
    ...
    for i in tqdm(range(len(As))):
        ...
        # x_feat2 = x_attribute_all[i]
        num_nodes = A.shape[0]
        num_edges = A.count_nonzero()
        if os.path.exists(data_dir1):
            A_new, num_of_one, X, P = reorder_adjacency_matrix_with_x(A,x_feat1)
        else:
            A_new, num_of_one, P = reorder_adjacency_matrix(A)
...
def main():
    ...
    print("exp with KM-Config two-split partitioning, with onehot node label/attributes")
    ### COLLAB PROTEINS
    ...
    for args.tau in [3,4,5,6,7,8,9,10]:
        ...
        embedding_file = args.dataset + '_LM' + '_' + str(args.k) + '_' + str(args.tau) + '_km_onehot.csv'
        ...
        file_out = '../../results/' + args.dataset + '_' + str(round(best_mean[idx],3)) + '_' + str(args.k) + '_' + str(args.tau) + '_CP_onehot.txt'
    ...
    print("exp with KM-Config two-split partitioning, with onehot node label/attributes, varying tau: ")
    print(best)
    best_file = '../../results/' + args.dataset + '_' + str(args.k) + '_CP_onehot_results.txt'
