...
def count_values_in_intervals(data, num_intervals):
    ...
    interval_ranges = [(intervals[i], intervals[i + 1]) for i in range(num_intervals)]
    interval_indices = {interval: [] for interval in interval_ranges}

    for i, value in enumerate(data):
        for j, interval in enumerate(interval_ranges):
            if interval[0] <= value < interval[1]:
                interval_indices[interval].append(i)
                break
...
def extract_LM_embeddings_label(args, root, dataset):
    ...
    for i in tqdm(range(len(As))):
        ...
        degree_feature = np.maximum(1, np.asarray(np.sum(A, axis=1)))
        x_feat = normalize_to_0_1(degree_feature)
...
def main():
    ...
    print("exp with CP partitioning, with node degree as feature")
    ...
    for args.tau in [3,4,5,6,7,8,9,10]:
        ...
        embedding_file = args.dataset + '_RWF' + '_' + str(args.k) + '_' + str(args.tau) + '_km_degree.csv'
        ...
        file_out = '../../results/' + args.dataset + '_' + str(round(best_mean[idx],3)) + '_' + str(args.k) + '_' + str(args.tau) + '_CP_with_degree_feature.txt'
    ...
    print("exp with CP partitioning, with node degree as feature, varying tau: ")
    print(best)
    best_file = '../../results/' + args.dataset + '_' + str(args.k) + '_CP_with_degree_feature_results.txt'
