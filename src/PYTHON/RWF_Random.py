...
def reorder_adjacency_matrix_with_x(adj_matrix, x_features):
    ...
    original_list = list(range(adj_matrix.shape[0]))
    # Randomly shuffle the list
    random.shuffle(original_list)
    sorted_indices = np.array(original_list)
...
def main():
    print("exp with random partitioning, without node feature")
    ### COLLAB PROTEINS
    ...
    for args.tau in [3, 4, 5, 6, 7, 8, 9, 10]:
        ...
        embedding_file = args.dataset + '_LM' + '_' + str(args.k) + '_' + str(args.tau) + '_Random.csv'
        ...
        if args.dataset in ["BANDPASS", "DBLP_v1", "reddit_threads", "twitch_egos", "github_stargazers",
                            "deezer_ego_nets"]:
            ...
            means, train_means, stds, train_stds, param_list = run_classification(args, feat, labels, kernel=kernel,
                                                                                  scaling=scaling)
        ...
        file_out = '../../results/' + args.dataset + '_' + str(round(best_mean[idx], 3)) + '_' + str(args.k) + '_' + str(args.tau) + '_random.txt'
    ...
    print("exp with random partitioning, without node feature, varying tau: ")
    print(best)

    best_file = '../../results/' + args.dataset + '_' + str(args.k) + '_R_results.txt'
